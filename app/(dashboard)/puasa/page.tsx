...
export default async function PuasaPage() {
  ...
  // 3. Waktu Saat Ini (Widget Header)
  // Perhatikan: getCurrentRamadhanDay sudah support Maghrib change
  ...
  // 4. Generate List Hari dengan Logika Lock TIME-BASED
  const days = Array.from({ length: 30 }, (_, i) => {
      const dayNum = i + 1
      const date = new Date(RAMADHAN_START_DATE_STR)
      date.setDate(date.getDate() + i) // Hari ke-1 = Tanggal Start (Tanpa mundur)
      ...
      // LOGIKA LOCK KHUSUS PUASA:
      // Buka kunci HANYA jika waktu sekarang >= 17:30 (Mendekati Maghrib) pada hari tersebut
      // atau jika hari tersebut sudah lewat.
      const unlockTime = new Date(date)
      unlockTime.setHours(17, 30, 0, 0) // Buka akses jam 17:30 sore
      ...
      <div className="bg-linear-to-br from-emerald-500 to-teal-900 rounded-2xl p-5 md:p-8 text-white shadow-xl relative overflow-hidden flex flex-col">
        ...
        <div className="relative z-10">
            <div className="mb-6">
                ...
                <p className="text-emerald-100 text-xs md:text-sm mt-1 leading-snug max-w-[90%]">
                    Jaga lisan, mata, dan hati untuk puasa yang sempurna.
                    <span className="block mt-1 text-emerald-300 text-[10px] italic opacity-80">(Ceklis terbuka setiap pukul 17.30 WIB)</span>
                </p>
            </div>
